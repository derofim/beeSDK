...
class AsyncWaitTimer
    : public std::enable_shared_from_this<AsyncWaitTimer>
{
public:
    typedef std::shared_ptr<AsyncWaitTimer> Ptr;
    typedef std::function<void (int32_t)> WaitFunc;
...
///////////////////////////注意timer设置后会立即回调一次//////////////////////////////////
/////////////invoke_times第一次回调值为0，其值代表经历了多少个周期////////////////////////
    void async_wait(WaitFunc wait_func)
    {
        if (false == is_cancel_ && dead_mill_seconds_) 
		{
...
#endif
            deadline_timer_.expires_from_now(immediately_?milliseconds(0):milliseconds(dead_mill_seconds_));
            deadline_timer_.async_wait(boost::bind(&AsyncWaitTimer::on_timer, 
                shared_from_this()));
        }
    }
    ...
    void cancel()
    {
        ...
        deadline_timer_.cancel(ignore);
    }

    void set_immediately(bool immediately){immediately_ = immediately;}
private:
    AsyncWaitTimer(boost::asio::io_service &ios)
        : deadline_timer_(ios), total_times_(-1), 
          is_cancel_(false), invoke_times_(0), dead_mill_seconds_(0),immediately_(true) { }

    void on_timer()
    {
        if (wait_func_ && false == is_cancel_) 
		{
            wait_func_(invoke_times_++);
            if (total_times_ < 0 
                || (total_times_ > 0 && invoke_times_ < total_times_)) 
			{
#if (BOOST_VERSION >= 104900)
                using std::chrono::milliseconds;
#else
				using boost::posix_time::milliseconds;
#endif
                deadline_timer_.expires_from_now(milliseconds(dead_mill_seconds_));
                deadline_timer_.async_wait(boost::bind(&AsyncWaitTimer::on_timer, 
                    shared_from_this()));
            } else 
			{
...
inline TimeType get_tick_count()
{
#ifdef WIN32
    return ::GetTickCount();
#else
