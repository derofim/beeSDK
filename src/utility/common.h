...
inline void simple_log(SimpleLogDst dst, const char *fmt, ...) {
    ...
    int32_t prefix_length = sprintf((char*)msg_str.data(), "[%s][%lu] ", time_string.c_str(), tid);
...
inline void simple_log(SimpleLogDst dst, int level, const char *source, int line, const char *fmt, ...) {
    ...
    size_t msg_length = 1024;
    std::string msg_str = std::string(msg_length, 0);
    int32_t prefix_length = sprintf((char*)msg_str.data(), "[%s][%lu][%s][%d] ", time_string.c_str(), tid, source, line);
    int32_t available_length = msg_length - prefix_length;
    if (available_length <= 0) {
        throw std::runtime_error{ "Too long log prefix." };
    }
...
const std::string kOneOff = "oneoff";
...
typedef struct SystemParam {
    ...
    std::string     log_path;    
    int32_t         log_level;
    int32_t         log_max_line;
    int32_t         log_volume_count;
    int32_t         log_volume_size;
