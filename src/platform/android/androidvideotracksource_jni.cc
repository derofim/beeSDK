...
static bee::AndroidVideoTrackSource* AndroidVideoTrackSourceFromJavaProxy(
    jlong j_proxy) {
  auto proxy_source = reinterpret_cast<webrtc::VideoTrackSourceProxy*>(j_proxy);
  return reinterpret_cast<bee::AndroidVideoTrackSource*>(
...
(JNIEnv* jni,
 ...
 jint rotation,
 jlong timestamp) {
    bee::AndroidVideoTrackSource* source =
...
(JNIEnv* jni, jclass, jlong j_source, jboolean j_success) {
  ...
  source->SetState(j_success
                       ? bee::AndroidVideoTrackSource::SourceState::kLive
                       : bee::AndroidVideoTrackSource::SourceState::kEnded);
}
...
(JNIEnv* jni, jclass, jlong j_source) {
  LOG(LS_INFO) << "AndroidVideoTrackSourceObserve_nativeCapturerStopped";
    bee::AndroidVideoTrackSource* source =
      AndroidVideoTrackSourceFromJavaProxy(j_source);
  source->SetState(bee::AndroidVideoTrackSource::SourceState::kEnded);
}
