...
#elif defined(IOS)
    m_Semaphore = dispatch_semaphore_create(initCount);
#else
...
inline Semaphore::~Semaphore()
{
	...
	m_Semaphore = NULL;
#elif defined(IOS)
    //dispatch_release(m_Semaphore); //Using arc.
#else
...
#elif defined(IOS)
    return (0 == dispatch_semaphore_wait(m_Semaphore, DISPATCH_TIME_FOREVER));
#else
...
#elif defined(IOS)
    return (0 == dispatch_semaphore_wait(m_Semaphore, dispatch_time(DISPATCH_TIME_NOW, nTimeout * NSEC_PER_MSEC)));
#else
	struct timespec ts;
