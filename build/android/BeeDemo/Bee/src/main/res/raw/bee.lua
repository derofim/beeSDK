...
function Party:create_session()
  local ret = false
  if (self.janus_ws) then
    self:log_info("Creating session")
    local transaction = self.video_room:create_transaction(true, handler(self, self.on_create_session))
    local msg = string.format("{\"bee\":\"create\",\"transaction\":\"%s\"}",transaction.id)
    ret = self.janus_ws:write(msg)
  end
  if (ret == false) then
    self:log_error("Create session fail, ws not created.")
...
function Party:on_create_session(success, json_data)
  if (success == false) then
    self:log_error("Create session fail.")
    self.video_room:on_party_create(35, self)
  else
    self.session_id = json_data.data.id
    local log = string.format("[index:%d][stream:%s] Create session success %d", self.session_id)
    LogInfo("%s", log)
    self:create_handle()
  end
end
...
function Party:create_handle()
  ...
  if (self.janus_ws) then
    ...
    local msg = string.format("{\"bee\":\"attach\",\"session_id\":%d,\"plugin\":\"bee.videoroom\",\"transaction\":\"%s\",\"opaque_id\":\"%s\", \"force-bundle\":true, \"force-rtcp-mux\":true}", self.session_id, transaction.id, opaque_id)
    ret = self.janus_ws:write(msg)
  end
  if (ret == false) then
    self:log_error("Create handle fail, ws not created.")
    self.video_room:on_party_create(38, self)
  end    
end
...
function Party:on_create_handle(success, json_data)
  if (success == false) then
    ...
    local log = string.format("[index:%d][stream:%s] Create handle success %d.", self.handle)
    LogInfo("%s", log)
    local ret = false
...
function Party:join()
  ...
  local msg = string.format("{\"bee\":\"message\",\"session_id\":%d,\"handle_id\":%d,\"transaction\":\"%s\",\"body\":{\"request\":\"join\",\"room_name\":\"%s\",\"ptype\":\"publisher\",\"stream_name\":\"%s\"}}", self.session_id, self.handle, transaction.id, self.janus_room_name, self.stream_name_internal)
  return self.janus_ws:write(msg)
end

function Party:on_joined(dummy, json_data)
  if (json_data.plugindata.data.videoroom == "joined") then
    self:log_info("Join success.")
    self.janus_keepalive_timer = Timer:open(30000, -1, handler(self, self.handle_janus_timeout))
    self:publish_local_feed()
  else
    self:log_error("Join failed.")
    self.video_room:on_party_create(35, self)
  end
end
...
function Party:publish_local_feed()
  ...
  if (self.rtc_conn ~= nil) then
    local ret = self.rtc_conn:create_offer(self.audio_source, self.video_source, handler(self, self.on_create_offer))
    if (ret == false) then
      self.video_room:on_party_create(41, self)
    end
  else
    self:log_error("Create offer failed, webrtc peerconnection open fail.")
...
function Party:on_ice_connection_change(new_state)
  ...
  if (new_state == kIceConnectionFailed or new_state == kIceConnectionDisconnected or new_state == kIceConnectionClosed) then
...
function Party:on_create_offer(local_jsep)
  ...
  if (local_jsep ~= nil and local_jsep ~= "") then
    ret = self:send_local_offer(local_jsep, handler(self, self.on_offer_answered))
  end
  if (ret == false) then
...
function VideoRoom:disconnect_stream(json_data)
  ...
  self:do_disconnect_stream(uid, stream_name, type)
end
...
function VideoRoom:on_party_closed(party)
  if (party ~= nil) then
    ...
    if (party.type == ePartyType_Local) then
      ...
      LogInfo("Stream %s close", party.stream_name_display)
      self.pull_stream_party_table[party.stream_name_display] = nil
      self.pull_stream_count = self.pull_stream_count - 1  
    end
...
function VideoRoom:do_disconnect_stream(uid, stream_name, type)
  local party = nil
  if (type == ePartyType_Local) then
    party = self.push_stream_party_table[stream_name]
    if (party ~= nil) then
...
function WhiteBoard:open()
  local protocols = {"janus-protocol"}
  if (Websocket:start(protocols) == false) then
...
function WhiteBoard:handle_msg_svc_joinback(cls_data)
  ...
  if (cls_data) then
    ...
    --self.msg_svc_list_timer = Timer:open(5000, -1, handler(self, self.handle_msg_svc_list_timeout))
    self.joined = true
  end
end
  ...
  for i = 1, #list do
    if (list[i] and list[i].uid and list[i].uid ~= self.uid) then
      ...
      if (self.member_table[member.uid] == nil) then
        ...
        self:report_data(data)
        self.member_table[member.uid] = member
      end   
    end
  end

  for k,v in pairs(self.member_table) do
    if(current_members[v.uid] == nil) then
      LogInfo("White board member uid %s leaved", v.uid)
      local data = string.format("{\"type\":%d,\"uid\":\"%s\"}", eWhiteBoardNotify_Remote_Leave, v.uid)
      self:report_data(data)
      self.member_table[k] = nil
    end
  end
end
