...
public class HudFragment extends Fragment {
  ...
  public View onCreateView(
    ...
    hudViewBwe.setMovementMethod(ScrollingMovementMethod.getInstance());
    hudViewConnection.setMovementMethod(ScrollingMovementMethod.getInstance());
    hudViewVideoSend.setMovementMethod(ScrollingMovementMethod.getInstance());
    hudViewVideoRecv.setMovementMethod(ScrollingMovementMethod.getInstance());
  ...
  public void updateEncoderStatistics(final StatsReport[] reports) {
    ...
    StringBuilder streamRecvStat = new StringBuilder();
    String fps = null;
    ...
    for (StatsReport report : reports) {
      if (report.type.equals("ssrc") && report.id.contains("ssrc") && report.id.contains("send")) {
        ...
        if (trackId != null /* && trackId.contains(PeerConnectionClient.VIDEO_TRACK_ID) */) { //TBD
        ...
        Map<String, String> reportMap = getReportMap(report);
        StringBuilder videoRecvStreamStat = new StringBuilder();
        StringBuilder audioRecvStreamStat = new StringBuilder();

        // Check if this stat is for track
        String mediaType = reportMap.get("mediaType");
        if (mediaType != null) {
          if (mediaType.equals("audio")) {
            audioRecvStreamStat.append("audio").append("\n");
            audioRecvStreamStat.append(report.id).append("\n");
            for (StatsReport.Value value : report.values) {
              String name = value.name.replace("goog", "");
              audioRecvStreamStat.append(name).append("=").append(value.value).append("\n");
            }
          }

          if (mediaType.equals("video")) {
            videoRecvStreamStat.append("video").append("\n");
            videoRecvStreamStat.append(report.id).append("\n");
            for (StatsReport.Value value : report.values) {
              String name = value.name.replace("goog", "");
              videoRecvStreamStat.append(name).append("=").append(value.value).append("\n");
            }
          }

          streamRecvStat.append(audioRecvStreamStat.toString() + videoRecvStreamStat.toString());
        }
    ...
    hudViewVideoRecv.setText(streamRecvStat.toString());
