...
static NSString *const kJanusKey = @"janus";
static NSString *const kTestStreamNameKey = @"test_stream_name_key";
static NSString *const kUidKey = @"uid";
static NSString *const kFpsKey = @"fps";
static NSString *const kLoopbackKey = @"loopback_key";
...
- (nullable NSString *)janus {
  return [self.storage objectForKey:kJanusKey];
}

- (void)setJanus:(nullable NSString *)url {
    [self.storage setObject:url forKey:kJanusKey];
    [self.storage synchronize];
}

- (nullable NSString *)testStreamName {
  return [self.storage objectForKey:kTestStreamNameKey];
}

- (void)setTestStreamName:(nullable NSString *)streamName {
  [self.storage setObject:streamName forKey:kTestStreamNameKey];
  [self.storage synchronize];
}
...
- (nullable NSNumber *)fps {
    return [self.storage objectForKey:kFpsKey];
}

- (void)setFps:(nullable NSNumber *)fps {
    [self.storage setObject:fps forKey:kFpsKey];
    [self.storage synchronize];
}

- (BOOL)loopback {
    return [self.storage boolForKey:kLoopbackKey];
}

- (void)setLoopback:(BOOL)loopback {
    [self.storage setBool:loopback forKey:kLoopbackKey];
    [self.storage synchronize];
}
