...
HttpRequest::HttpRequestPtr HttpRequest::create_from_url(const std::string& url, const std::string& refer_url, int64_t range_beg, int64_t range_end) {
    HttpRequest::HttpRequestPtr prequest;
    std::string protocol, host, port, path;
    std::tie(protocol, host, port, path) = HttpRequest::ParseUrl(url);
    ...
    if (protocol == "https") {
        prequest->is_https_ = true;
    }

    return prequest;
}

std::tuple<std::string, std::string, std::string, std::string> HttpRequest::ParseUrl(const std::string & url) {
    ...
    if (pos_host != std::string::npos  && pos_slash != std::string::npos && pos_host < pos_slash ) {
    ...
    return std::make_tuple(protocol, host_str, port_str, path_str);
}
...
std::string HttpRequest::get_header(const std::string& name) const {
    std::map<std::string, std::string>::const_iterator it = m_header_fields_map.find(name);
...
void HttpRequest::set_header(const std::string& name, const std::string& val) {
    std::map<std::string, std::string>::iterator it_map = m_header_fields_map.find(name);
...
bool HttpRequest::assign(std::string &data)
{
    bool ret = true;
    do 
    {
        ...
        size_t content_length = atoi(get_header("Content-Length").c_str());
        ...
        m_content = data.substr(pos,content_length);
        data.erase(0,content_length + pos);
    } while (0);
